...
contract("CryptoZombies", (accounts) => {
  ...
  it("check zombie length", async () => {
    let result = await contractInstance.getZombiesLength();
    expect(result.toNumber()).to.equal(0);
    await contractInstance.createRandomZombie(zombieNames[0], {
      from: alice,
    });
    result = await contractInstance.getZombiesLength();
  ...
  it("should be able to create a new zombie", async () => {
    ...
    expect(result.logs[1].args.name).to.equal(zombieNames[0]);
  });
  ...
  context("with the single-step transfer scenario", async () => {
    it("should transfer a zombie", async () => {
      ...
      const zombieId = result.logs[1].args.zombieId.toNumber();
      await contractInstance.safeTransferFrom(alice, bob, zombieId, {
        from: alice,
      });
  ...
  it("zombies should be able to attack another zombie", async () => {
    ...
    const firstZombieId = result.logs[1].args.zombieId.toNumber();
    result = await contractInstance.createRandomZombie(zombieNames[1], {
      from: bob,
    });
    const secondZombieId = result.logs[1].args.zombieId.toNumber();
    await time.increase(time.duration.days(1));
    result = await contractInstance.attack(firstZombieId, secondZombieId, {
      from: alice,
    });
    expect(result.receipt.status).to.equal(true);
  });
